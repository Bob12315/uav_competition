...
- Entry point: `main.py` ties fake FC, vision, mission loop, and HUD for the desktop demo.
...
- Functional modules: `app/io` (MAVLink clients, RC, gimbal, dropper), `app/vision` (camera + detection), `app/mission` (state machine, align control), `app/hud` (OSD renderer), `app/telemetry` (CSV logging).
- Simulators: `sims/sim_vision.py`, `sims/sim_mission.py`, `sims/sim_hud.py` for module-level dry runs.
...
- Module demos: `python sims/sim_vision.py`, `python sims/sim_mission.py`, `python sims/sim_hud.py` to validate each subsystem independently before hardware.
...
- Keep module boundaries clean: IO does MAVLink only; Mission decides; Vision detects; HUD renders; no cross-importing private helpers.
...
- Keep PRs scoped: one feature or fix; link issues/tasks; add screenshots or logs for HUD/vision changes.
